...
class VectorIndex():
    ...
    def generate_index(self):
        ...
        if type(self.iterable) == QuerySet:
            self.vectors = np.array(self.iterable.values_list('embedding_all_mpnet_base_v2', flat=True)).astype(np.float32)
        else:
    ...
    def query_min_distance(self, query: str, k: int = 1, min_distance: float = .21):
        ...
        Args:
            query (str): The string to find closest matches for
            k (int, optional): Number of results to return. Defaults to 1.
            min_distance (float, optional): Minimum distance between matches. Ranges from 0 to 1, 0 returning all results and 1 returning none. Defaults to 0.1.
        ...
        start = time.perf_counter()
        query_vector = self.embedding_model.encode([query])
        values, indices = self.index.search(query_vector, k*10) # times 10 cuz we remove a ton of matches in the next step
        ...
        # results = [(self.iterable[indice], value) for value, indice in zip(values.tolist()[0], indices.tolist()[0])] # to compare to the new results
        clean_results = [(self.iterable[indice], value) for value, indice in zip(values.tolist()[0], cleaned_indices)]
        return clean_results[:k]
