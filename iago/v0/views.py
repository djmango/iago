...
class contentSkillSearch(views.APIView):
    ...
    def get(self, request):
        ...
        start = time.perf_counter()
        if strict:
            content = list(ScrapedArticle.objects.filter(skills=skills).values('uuid', 'title', 'url', 'skills', 'thumbnail', 'updated_on'))
        else:
            content = list(ScrapedArticle.objects.filter(skills__in=skills).values('uuid', 'title', 'url', 'skills', 'thumbnail', 'updated_on'))
